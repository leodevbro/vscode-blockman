...
export default class SingularBracketGroup implements IBracketManager {
    ...
    constructor(
        ...
        if (previousState !== undefined) {
            this.allLinesOpenBracketStack =
                previousState.currentOpenBracketColorIndexes;
    ...
    public addOpenBracket(token: Token, colorIndex: number) {
        const openBracket = new Bracket(token, "red");
    ...
    public addCloseBracket(token: Token) {
        if (this.allLinesOpenBracketStack.length > 0) {
            if (
                this.allLinesOpenBracketStack[
                    this.allLinesOpenBracketStack.length - 1
        ...
        const orphan = new Bracket(token, "red");
